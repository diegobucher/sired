...
function executaEnterCampo(event, id) {	
...
function clickAccordion(td) {
    $(td).parents(".accordion-heading").toggleClass("collapsed");
    $(td).parents(".accordion-heading").next().toggleClass("collapse");
    return false;
}

var siredStatusBlocked = false;

function showStatus(block) {
	if (typeof block !== 'undefined') {
		siredStatusBlocked = block;
	} 
...
function hideStatus(block) {
	if (typeof block !== 'undefined') {
		siredStatusBlocked = block;
	} 	
	
	if (!siredStatusBlocked) {
		$("#idLoad").hide();
		$.unblockUI();
	}
}
