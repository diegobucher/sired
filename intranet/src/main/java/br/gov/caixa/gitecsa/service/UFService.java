...
public class UFService extends AbstractService<UFVO> implements Serializable {
    ...
    public List<UFVO> findAssociadoUnidade() {
        Criteria criteria = getSession().createCriteria(UFVO.class).createAlias("unidades", "unidade", JoinType.INNER_JOIN);
    ...
    public List<UFVO> findAssociadoCTDA() {
        Criteria criteria = getSession().createCriteria(UFVO.class).createAlias("ctdaUfs", "ctda", JoinType.INNER_JOIN);
